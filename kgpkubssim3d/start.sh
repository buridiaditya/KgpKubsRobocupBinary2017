...
usage()
{
  (echo "Usage: $0 [options]"
   ...
   echo "  -pf, --paramsfile FILENAME     name of a parameters file to be loaded (default: paramfiles/defaultParams.txt)"
   echo "  -no, --num_players NUM_PLAYERS number of players "
   echo "  -ty, --type TYPE               type of player, if one player selected ") 1>&2
}
...
for ((i=1;i<=$NUM_PLAYERS;i++)); do
  ...
  1|2)
      if [ $NUM_PLAYERS -eq 1 ];
      then
      echo "Running agent No. $i -- Type $TYPE and NUM_PLAYERS = 1"
